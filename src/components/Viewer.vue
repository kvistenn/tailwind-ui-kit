<template>
    ...
    <div class="flex flex-col border border-gray-200 rounded-lg p-2 gap-2" :class="{ 'h-full': fullscreen }">
        ...
        <div v-show="!toggle" class="block flex-auto p-4 viewer min-h-36 rounded-md border border-gray-200 dark:border-none dark:bg-gray-900">
            <div class="w-full h-full flex-wrap relative mx-auto transition-all" :class="[' ' + gap, device == 0 ? ' max-w-full' : device == 1 ? ' max-w-screen-md' : device == 2 ? ' max-w-screen-xs' : null]">
                <dynamic-component
                    v-if="items.length > 0"
                    v-for="item in items"
                    :component="item"
